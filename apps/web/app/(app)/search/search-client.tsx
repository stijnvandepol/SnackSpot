...
export function SearchClient() {
  ...
  return (
    // The <h1> and the intro live in the server page (page.tsx) so they reach crawlers
    // without waiting for hydration; this component owns the interactive part only.
    <div>
      {showKeyboardTip && <p className="mb-3 text-xs text-snack-muted">Tip: druk op / om te zoeken.</p>}
      ...
      <div className="card mb-4 p-4">
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
          <input
            ...
            placeholder="Zoek op zaak of gerecht..."
            value={q}
            onChange={(e) => setQ(e.target.value)}
            autoFocus={showKeyboardTip}
            aria-label="Zoek op zaak of gerecht"
          />
      ...
      {(searched || hasActiveTag) && (
        <div className="mb-4 rounded-xl border border-snack-border bg-snack-surface px-4 py-3 text-sm text-snack-muted" aria-live="polite">
          {searched && <span>Resultaten voor &ldquo;{q}&rdquo;.</span>}
          {!searched && hasActiveTag && <span>Reviews met het label {getReviewTagLabel(activeTag).toLowerCase()}.</span>}
        </div>
      )}
      ...
      {showTagResults && (
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-snack-muted">{tagResults.length} {tagResults.length === 1 ? 'review' : 'reviews'} met dit label.</p>
          </div>
